...
import { CalendarDays, FileText, CheckCircle, SettingsIcon } from "lucide-react"
import {
  Sidebar,
  SidebarContent,
  SidebarFooter,
...
export function AppSidebar({ user, onSettingsClick }: AppSidebarProps) {
  ...
  return (
    <Sidebar
      className="border-r border-zinc-200 bg-zinc-50 text-zinc-900 dark:border-zinc-800 dark:bg-black dark:text-white"
      collapsible="icon"
    >
      <SidebarHeader className={`border-b border-zinc-200 dark:border-zinc-800 p-0 ${isCollapsed ? "py-4" : ""}`}>
        <div className={`flex flex-col items-center px-6 py-4 text-center ${isCollapsed ? "py-0" : ""}`}>
          <img
            src="/blank-profile-icon.png"
            alt="Profile"
            className={`${isCollapsed ? "h-8 w-8 mb-0" : "mb-3 h-16 w-16"} rounded-full border border-zinc-300 dark:border-zinc-600`}
          />

          {/* Hide these elements when collapsed */}
          {!isCollapsed && (
            <>
              <h3 className="text-lg font-semibold">{user ? `${user.name} ${user.surname}` : "Loading..."}</h3>
              <p className="mt-1 text-center text-xs text-zinc-500 dark:text-zinc-500">
                {user ? getRoleString(user) : ""}
              </p>
              <button
                onClick={onSettingsClick}
                className="mt-4 w-fit rounded bg-zinc-200 px-3 py-2 text-sm hover:bg-zinc-300 dark:bg-zinc-700 dark:hover:bg-zinc-600"
              >
                Update Info
              </button>
            </>
      ...
      <SidebarFooter className="mt-auto border-t border-zinc-200 dark:border-zinc-800 p-4">
        ...
        {!isCollapsed && (
          <div className="text-xs text-zinc-500 dark:text-zinc-500">{user && `Logged in as ${user.email}`}</div>
        )}
      </SidebarFooter>
      <SidebarRail />
    </Sidebar>
